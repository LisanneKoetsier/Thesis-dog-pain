...
def build_model(cfg, decision_tree):
    ...
    #print(f"this is the model: {getattr(model.clstm.convlstm.cell_list, '1')}") #this is one target layer.
...
def val_epoch(
    ...
    for cur_iter, (inputs, labels, ethogram, start_name) in enumerate(tqdm(test_loader)):
        ...
        #print("this is the validation batch size", inputs[0].size(0))
        batch_size = inputs[0].size(0) # TODO figure out what it needs to be to run, it was this: inputs[0].size(0)
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, ethogram, start_name) in enumerate(tqdm(train_loader)):
        ...
        loss.backward()
        optim.step()
        #print("this is the train batch size", inputs[0].size(0))
...
def train_net(cfg):
    ...
    if cfg.MODEL.DECISION_TREE:
        ...
        for i, tree in enumerate(rf_classifier.estimators_):
            ...
            file_path = os.path.join("decision_trees", f"decision_tree.pdf")
