...
class TimeDistributed(nn.Module):
    """
    Allows to apply a layer to every temporal part of an input. This helps with video data
    """
...
class Head(nn.Module):
    ...
    def forward(self, x):
        #print(f"Input to fc layer shape: {x.shape}")
        out = self.fc(x)
        if self.two_out:
            out2 = self.aux_fc(x)
            return [out, out2]
...
class Concat(nn.Module):
    def __init__(self, dim1, dim2, dim3, cfg):
        super(Concat, self).__init__()
        self.model_type = cfg.MODEL.TYPE
        if self.model_type == "ethogram":
            self.head  = Head(cfg, dim1+dim2+dim3)#change to dim3 with linear
        else:
