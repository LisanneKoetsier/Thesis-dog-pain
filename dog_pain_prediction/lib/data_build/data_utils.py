...
def aug_param_builder(clip_inf, video_name):
    ...
    rotate_prob, flip_prob, blur_prob, jitter_prob = np.random.uniform(), np.random.uniform(), np.random.uniform(), np.random.uniform()
    aug_para = [
        rotate_degree, rotate_prob, flip_prob, blur_prob, jitter_prob, img_w, img_h
    ]
    return aug_para
...
def gaussian_blur(prob, images):
    if prob < 0.2:
        sigma = random.random() * 1.9 + 0.1
        gaussian_transformer = T.GaussianBlur(kernel_size=3, sigma=sigma)  # Adjust kernel_size if needed
        blurred_images = []
        for img in images:
            blurred_img = gaussian_transformer(T_func.to_pil_image(img))
            blurred_images.append(T_func.to_tensor(blurred_img))
        return torch.stack(blurred_images)
    else:
        return images

def color_jitter(prob, images):
    worker = T.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1)
    if prob < 0.5:
        # Apply ColorJitter and convert back to tensor
        aug_images = torch.stack([T_func.to_tensor(worker(T_func.to_pil_image(img))) for img in images])
        return aug_images
    else:
        return images
...
def frame_augmentation(frames, aug_para):
    ...
    #frames = gaussian_blur(aug_para[3], frames)
    frames = color_jitter(aug_para[4], frames)
    return frames
...
class Dataset_inf:
    def __init__(self, cfg):
        if not cfg.DATA.REQUIRE_AUX:
            self.label_info = {"pain": 0, "not_pain": 0} #TODO: figure out why these are all zeros
            self.two_label = 0
...
def weightedsample(clip_inf, data_type):
    #print(f"this is clip_inf: {clip_inf}")   
    clip_inf = np.array(clip_inf)
    label_list = clip_inf[:, -2] if data_type == "aux" else clip_inf[:, -2] #REMINDER: changed to -2 such that it does not look at the ethogram
    #print(f"this is the label_list: {label_list}")
...
def loss_weight(clip_inf, data_type, balance_policy):
    ...
    if data_type in ["simple", "diff"]:
        if balance_policy == 2:
            ...
            labels = clip_inf[:, -2] #-1 before ethogram
        ...
        out_weight = []
        label_set = clip_inf[:,-2:].T #TODO change this index when using ethogram
