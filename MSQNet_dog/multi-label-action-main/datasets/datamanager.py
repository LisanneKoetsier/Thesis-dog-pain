...
from datasets.datasets import AnimalKingdom, Dog, Dog_one_ethogram_dimension, Charades, Hockey, Thumos14, Volleyball
...
class DataManager():
    def __init__(self, args, path):
        ...
        self.idle = args.idle
        self.one_action = args.one_action
        self.second_dimension = args.second_dimension

    def _check(self,):
        datasets_list = ["animalkingdom", "ava", "charades", "hockey", "thumos14", "volleyball", "dog"]
    ...
    def get_num_classes(self,):
        ...
        elif(self.dataset == "dog" and self.idle == False): return 9 #if predicting all behaviors
        elif(self.dataset == "dog" and self.idle == True): return 4 #if only predicting idle 
        elif(self.dataset == "dog" and self.one_action): return 3 #if predicting one ethogram 
        elif(self.dataset == "dog" and self.second_dimension): return 6 #if predicting second behaviors, such as decreased weight bearing when walking
    ...
    def get_act_dict(self,):
        self._check()
        print(f"this is the one action in datamanager: {self.one_action}")
        print(f"this is the second dimension in datamanager: {self.second_dimension}")
        ...
        dog_dict = {'standing still': 0 , 'walking': 1, 'trotting':2, 'circling': 3, 'jumping up': 4, 'sitting': 5, 'sitting abnormally': 6, 'lying down': 7, 'obscured': 8, 'ul': 9}
        dog_dict_idle = {'standing still': 0 ,'sitting': 1, 'sitting abnormally': 2, 'lying down': 3, 'ul': 4}
        dog_dict_one_action = self.generate_action_dict(self.one_action)
        dog_dict_second_dimension = {'decreased weight bearing': 0, 'normal weight bearing': 1, 'rigid posture': 2, 'rigid gait': 3, 'no rigidness': 4, 'ul': 5}
        ...
        print(f"this is the boolean: {self.dataset == "dog" and self.one_action is not None}")
        if(self.dataset == "animalkingdom"): return animalkingdom_dict
        elif(self.dataset == "dog" and self.one_action is not None): return dog_dict_one_action
        elif(self.dataset == "dog" and self.idle == True): return dog_dict_idle
        elif(self.dataset == "dog" and self.second_dimension == True): return dog_dict_second_dimension
        elif(self.dataset == "dog" and self.idle == False): return dog_dict
    ...
    def generate_action_dict(self,action):
        action_dict = {}
        if action is not None:
            opp_action = "not " + action
            action_dict = {action: 0, opp_action: 1, 'ul': 2}
        return action_dict
    ...
    def custom_collate(self, batch):
        process_data = torch.stack([item[0] for item in batch], dim=0)
        labels = torch.stack([item[1] for item in batch], dim=0)
        tuples = [item[2] for item in batch]
        img_names = [item[3] for item in batch]
        return process_data, labels, tuples, img_names
    ...
    def get_train_loader(self, train_transform, drop_last=False):
        ...
        print(f"this is the train action dict: {act_dict}")
        if(self.dataset == 'animalkingdom'):
            ...
            shuffle = False
        elif(self.dataset == 'dog' and self.one_action is not None):
            train_data = Dog_one_ethogram_dimension(self.path, act_dict, total_length=self.total_length, transform=train_transform, mode='train', unsupervised=self.unsupervised, one_action=self.one_action)
            sampler = RandomSampler(train_data, num_samples=2500) #TODO: look into if the num_samples are not too much
            if self.distributed:
                sampler = DistributedSamplerWrapper(sampler, shuffle=True)
            shuffle = False
        elif(self.dataset == 'dog'):
            train_data = Dog(self.path, act_dict, total_length=self.total_length, transform=train_transform, mode='train', unsupervised=self.unsupervised, idle=self.idle, second_dimension=self.second_dimension)
            sampler = RandomSampler(train_data, num_samples=2500) #TODO: look into if the num_samples are not too much
            if self.distributed:
                sampler = DistributedSamplerWrapper(sampler, shuffle=True)
            shuffle = False
        ...
        train_loader = DataLoader(train_data, batch_size=self.batch_size, shuffle=shuffle, num_workers=self.num_workers, sampler=sampler, pin_memory=False, drop_last=drop_last, collate_fn=self.custom_collate)
        return train_loader
    ...
    def get_test_loader(self, test_transform, drop_last=False):
        ...
        if(self.dataset == 'animalkingdom'):
            ...
            test_data = Dog_one_ethogram_dimension(self.path, act_dict, total_length=self.total_length, transform=test_transform, mode='test', unsupervised=self.unsupervised, one_action=self.one_action)
        elif(self.dataset == 'dog'):
            test_data = Dog(self.path, act_dict, total_length=self.total_length, transform=test_transform, mode='test', unsupervised=self.unsupervised, idle=self.idle, second_dimension=self.second_dimension)
        elif(self.dataset == 'charades'):
        ...
        shuffle = False
        test_loader = DataLoader(test_data, batch_size=self.batch_size, shuffle=shuffle, num_workers=self.num_workers, sampler=sampler, pin_memory=True, drop_last=drop_last, collate_fn=self.custom_collate)
        return test_loader