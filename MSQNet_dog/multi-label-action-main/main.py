...
def main(args):
    ...
    if args.dataset in ['animalkingdom', 'charades', 'hockey', 'volleyball']:
        ...
        print(f"normal cross entropy loss is used")
        criterion = nn.CrossEntropyLoss()

    # evaluation metric
    if args.dataset in ['animalkingdom', 'charades', 'dog']:
    ...
    model_args_unsupervised = (train_loader, val_loader, criterion, eval_metric, class_list, args.test_every, args.distributed, device, args.unsupervised, args.idle, args.one_action, args.second_dimension)
    if args.model == 'convit':
        ...
        executor = TimeSformerCLIPInitVideoGuideExecutor(*model_args_unsupervised)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--unsupervised", default=False, type=bool, help="supervised or unsupervised")
    parser.add_argument("--idle", default=False, help="whether or not to only predict the subclasses of standing still")
    parser.add_argument("--one_action", default = None, help="whether you want to run the model with one ethogram dimension only, fill in the action as a string")
    parser.add_argument("--second_dimension", default = True, help="whether you want to run second dimension behaviors")
